...
- `agents`: `Observation` を受け取り、構造化 action を返す。現在は provider 呼び出しを行わない `FakeLlmAgent` のみ。
- `interfaces/api`: Django / DRF の公開 API。HTTP 入出力、Django config、例外変換だけを扱う。
- `interfaces/cli`: 公開 HTTP API だけを呼ぶ CLI。domain / agents / usecase を直接 import しない。
