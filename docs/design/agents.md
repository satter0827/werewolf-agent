...
LLM の自由文は直接ゲーム操作へ変換しない。生JSONをPydantic schemaで検証し、
...
`DecisionModel`、trace、player portを定義する。`adapters.llm`はLangChain型への変換と
...
発言の`focus_id`と`evidence_id`は公開発言記録へ保存し、次の発言・投票で本人を含む全playerが
同じ公開立場を参照する。これによりAgent固有の非公開memoryを追加せず、発言変更と投票整合を
公開情報だけで評価する。
...
Agent pipeline実行前後だけ短いtransactionを開き、model待機中はDB connectionを保持しない。
