...
from werewolf_agent.adapters.supabase.worker.llm_trace import SupabaseLlmTraceSink
from werewolf_agent.adapters.supabase.worker.repository import (
...
def _create_game(
    ...
    result = usecases.create_game(service, _create_command(create_request, settings))
...
def _advance_game(
    ...
    runtime = AgentRuntime(
        config=build_llm_provider_config(settings),
    ...
    result = advance_agent_game(
        service,
        usecases.AdvanceGameCommand(game_id=game_id),
        runtime=runtime,
    )
...
def _submit_action(
    ...
    service = _service(connection, settings)
    result = usecases.submit_player_action(
        service,
        usecases.PlayerActionCommand(
            game_id=game_id,
            player_id=player_id,
...
def _materialize_private_views(connection: Any, settings: AppSettings, game_id: str) -> None:
    ...
    for participant in participants:
        ...
        if player_id == "observer":
            continue
        observation = usecases.get_player_observation(
            service,
            usecases.GetPlayerObservationQuery(
                game_id=game_id,
                player_id=player_id,
...
def _materialize_reveal_view(
    connection: Any,
    settings: AppSettings,
    service: usecases.UsecaseContext,
    game_id: str,
) -> int:
    ...
    reveal = usecases.get_game_reveal(
        service,
        usecases.GetGameRevealQuery(game_id=game_id),
    )
...
def _current_game_version(service: usecases.UsecaseContext, game_id: str) -> int:
    game = usecases.get_game(service, usecases.GetGameQuery(game_id=game_id))
...
def _service(
    ...
    *,
    ...
    return usecases.UsecaseContext(
...
def _create_command(
    ...
    settings: AppSettings,
) -> usecases.CreateGameCommand:
    return usecases.CreateGameCommand(
        seed=request.seed,
