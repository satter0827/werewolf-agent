...
def default_setup_options(
    ...
    if not player_definitions.contains_strategy(config.default_agent_strategy_id):
        raise ValueError(message_unknown_agent_strategy(config.default_agent_strategy_id))
    ...
    return GameSetupOptionsResult(
        ...
        agent_strategies={
            ...
            for strategy in player_definitions.agent_strategies.values()
        },
    )
