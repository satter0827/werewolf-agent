"""State and value models for the deterministic headless game."""
...
from werewolf_agent.domain._model import DomainModel, non_blank, optional_non_blank
...
class AbilityDefinition(DomainModel):
    """Behavior values for one registered role ability."""
    ...
    def validate_registered_policies(self) -> Self:
        """Reject ability algorithms not implemented by the standard core."""
        ...
        supported_targets = {"none", "alive", "other_alive", "other_alive_non_pack"}
        if self.target_policy not in supported_targets:
...
class _DomainModel(DomainModel):
...
class LocalRules(_DomainModel):
    ...
    def validate_tie_resolution(self) -> Self:
        """Require exactly one configured tie-resolution algorithm."""
        enabled = (
...
class GameConfig(_DomainModel):
    ...
    def validate_counts(self) -> Self:
        ...
        referenced_abilities = {
            ability for role in self.roles.roles.values() for ability in role.abilities
        }
        unknown_abilities = sorted(referenced_abilities - set(self.abilities))
        if unknown_abilities:
            raise ValueError(message_unsupported_abilities(unknown_abilities))
        expected_phases = {Phase.NIGHT, Phase.DAY_DISCUSSION, Phase.VOTING}
        if (
            len(self.phase_order) != len(expected_phases)
            or set(self.phase_order) != expected_phases
        ):
            raise ValueError(
...
class GameSnapshot(_DomainModel):
    """Serializable full aggregate state for application boundaries."""
    ...
    def is_finished(self) -> bool:
        """Return whether no further phase transition is possible."""
        return self.phase is Phase.FINISHED

    @property
    def winner_id(self) -> str | None:
        """Return the configured winning faction id, when decided."""
...
class Observation(_DomainModel):
    ...
    legal_targets: dict[ActionType, list[str]] = Field(default_factory=dict)
    history: GameHistory = Field(default_factory=GameHistory)
    win_result: WinResult | None = None
...
class GameSetup(_DomainModel):
    """Players used to create one game."""

    players: tuple[Player, ...]


GameState = GameSnapshot
GameView = Observation
GameEvent = DomainEvent
