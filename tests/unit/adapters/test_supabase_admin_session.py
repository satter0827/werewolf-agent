...
def _verifier(handler: httpx.MockTransport) -> SupabaseAdminSessionVerifier:
...
def test_live_admin_session_requires_current_user_and_server_owned_role() -> None:
    ...
    try:
        assert verifier.verify("access-token", expected_user_id="user-1") is True
        assert verifier.verify("access-token", expected_user_id="other-user") is False
    finally:
        verifier.close()
...
def test_revoked_or_rejected_session_is_not_admin(status_code: int) -> None:
    ...
    try:
        assert verifier.verify("access-token", expected_user_id="user-1") is False
    finally:
        verifier.close()
...
def test_auth_service_failure_is_distinct_from_rejected_session() -> None:
    ...
    try:
        with pytest.raises(AdminSessionUnavailable):
            verifier.verify("access-token", expected_user_id="user-1")
    finally:
        verifier.close()
