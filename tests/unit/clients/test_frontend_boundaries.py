...
def test_human_interface_client_port_excludes_administrator_operations() -> None:
    ...
    for path in (PACKAGE / "clients" / "streamlit").rglob("*.py"):
...
def test_action_text_limit_is_shared_by_api_react_and_streamlit() -> None:
    ...
    api_settings = (PACKAGE / "settings" / "sections" / "api.py").read_text(encoding="utf-8")
    streamlit_app = "\n".join(
        (PACKAGE / "clients" / "streamlit" / "views" / name).read_text(encoding="utf-8")
        for name in ("runtime.py", "game.py")
    )
    ...
    assert "WEREWOLF_API_MESSAGE_MAX_CHARS" in api_settings
    assert "WEREWOLF_STREAMLIT_MESSAGE_MAX_CHARS" not in api_settings
