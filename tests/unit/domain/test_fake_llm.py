...
def test_fake_llm_returns_role_specific_night_decisions() -> None:
    wolf_decision = choose_fake_llm_decision(
        "p1",
        observation(
            player_id="p1",
    ...
    seer_decision = choose_fake_llm_decision(
        "p2",
        observation(player_id="p2", role=AgentRole.SEER),
        rng=random.Random(1),
    )
    knight_decision = choose_fake_llm_decision(
        "p3",
        observation(player_id="p3", role=AgentRole.KNIGHT),
        rng=random.Random(1),
    )
    villager_decision = choose_fake_llm_decision(
        "p4",
...
def test_fake_llm_is_seed_deterministic_for_same_observation() -> None:
    ...
    decision_a = choose_fake_llm_decision("p1", agent_observation, rng=random.Random(99))
    decision_b = choose_fake_llm_decision("p1", agent_observation, rng=random.Random(99))

    assert decision_a == decision_b


def test_fake_llm_day_and_vote_decisions_match_phase() -> None:
    speech = choose_fake_llm_decision(
        "p2",
        observation(player_id="p2", role=AgentRole.SEER, phase=AgentPhase.DAY_DISCUSSION),
        rng=random.Random(3),
    )
    vote = choose_fake_llm_decision(
        "p2",
...
def test_fake_llm_passes_when_observation_is_not_for_player() -> None:
    decision = choose_fake_llm_decision(
        "p1",
        observation(player_id="p2", role=AgentRole.SEER),
        rng=random.Random(3),
    )
