...
from werewolf_agent.entrypoint.cui import commands as cui_commands
from werewolf_agent.entrypoint.cui.app import app
...
def test_run_app_command_handles_app_error_safely(caplog: pytest.LogCaptureFixture) -> None:
    ...
    with caplog.at_level(logging.INFO, logger="werewolf_agent.entrypoint.cui.errors"):
...
def test_play_command_handles_data_source_problem_safely(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        cui_commands,
        "build_game_api",
        lambda _settings: FailingGameApi(),
    )
