...
from werewolf_agent.interfaces.cli import commands as cui_commands
...
def test_run_app_command_handles_app_error_safely(caplog: pytest.LogCaptureFixture) -> None:
    ...
    with caplog.at_level(logging.INFO, logger="werewolf_agent.interfaces.cli.errors"):
...
def test_setup_options_requires_supabase_client_env_before_building_client(
    ...
    monkeypatch.setattr(cui_commands, "build_game_client", fail_build_game_client)
...
def test_run_app_command_logs_sanitized_error_message(caplog) -> None:
    ...
    with (
        caplog.at_level(logging.INFO, logger="werewolf_agent.interfaces.cli.errors"),
