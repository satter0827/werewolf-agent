...
def test_game_application_config_is_built_from_application_settings() -> None:
    settings = AppSettings(
        _env_file=None,
        game_min_players=5,
        game_max_players=8,
    ...
    assert application_config.min_players == 5
    ...
    assert application_config.timeline_default_limit == 100
    assert application_config.timeline_max_limit == 500
...
def test_game_settings_load_from_environment(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv(
        ...
        "src/werewolf_agent/agents/resources/llm/decision_graphs.toml",
    )
    ...
    assert (
        ...
        == repository_root() / "src/werewolf_agent/agents/resources/llm/decision_graphs.toml"
    )
...
""".strip(),
    ...
    assert sorted(llm_definitions.players.players) == ["quiet"]
    assert llm_definitions.prompt.name == "test"


def test_invalid_definition_values_fail_in_resource_adapter(tmp_path: Path) -> None:
    roles_file = tmp_path / "roles.toml"
    roles_file.write_text(
        """
[roles.plain]
...
def test_unknown_role_ability_fails_during_definition_load(tmp_path: Path) -> None:
    roles_file = tmp_path / "roles.toml"
    roles_file.write_text(
        """
[roles.villager]
...
""".strip(),
    ...
    with pytest.raises(ValueError, match="roles references unknown abilities: teleport"):
        build_game_definitions(settings)


def test_unknown_default_setup_preset_fails_during_definition_load() -> None:
    settings = AppSettings(_env_file=None, game_default_setup_preset_id="missing")
    with pytest.raises(ValueError, match="Unknown setup preset: missing"):
        build_game_definitions(settings)


def test_missing_local_rule_flags_fail_during_definition_load(tmp_path: Path) -> None:
    rules_file = tmp_path / "rules.toml"
    rules_file.write_text(
        """
[local_rules]
...
""".strip(),
    ...
    settings = AppSettings(_env_file=None, game_rules_file=str(rules_file))
    with pytest.raises(ValueError, match="day_speech_limit_per_player"):
        build_game_definitions(settings)


def test_missing_default_role_counts_fail_during_definition_load(tmp_path: Path) -> None:
    roles_file = tmp_path / "roles.toml"
    roles_file.write_text(
        """
[roles.villager]
